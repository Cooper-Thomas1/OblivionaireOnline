...
- Session expiry time was not specified in the project specifications, so the session time is set as the expiration time of the account. This made the most sense in terms of higher-level users of the *OO* system, whereby they can alter the expiration time in the account state which will consequently affect any sessions that are produced by logins on an account. This made the most sense to our group so to be our "reasonable assumption".
