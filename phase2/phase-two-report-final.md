...
- Before casting `account_id` to `int`, we check for overflow. This prevents session state corruption or TOCTOU-style attacks involving unsafe type conversions.
...
- Session expiry time was not specified in the project specifications, so the session time is the default value of unlimited.
